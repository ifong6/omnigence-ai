...
from app.dto.agent_dto import AgentFlowRequestDTO, FinanceAgentFlowRequestDTO
...
class AgentController(BaseController):
    ...
    def handle_human_in_the_loop(request: HumanInTheLoopRequest):
        ...
        try:
            ...
            resume_result = resume_agent(request)
    ...
    def call_finance_agent(request: UserRequest):
        BaseController.log_request(
         ...
         # 🔁 Adapt UserRequest to AgentFlowRequestDTO
        flow_request = AgentFlowRequestDTO(
            session_id=request.session_id,
            user_input=request.message,        # or user_input / text, etc.
            agent_type="finance",
        )

        try:
            # Execute finance agent workflow
            result = finance_agent_flow(flow_request)
