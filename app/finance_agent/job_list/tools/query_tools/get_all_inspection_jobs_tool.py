"""
Tool for retrieving all INSPECTION jobs from the Finance database.
"""
from database.supabase.db_connection import execute_query
from database.supabase.db_enum import DBTable_Enum


def get_all_inspection_jobs_tool() -> str:
    """
    Get all INSPECTION jobs from the database, ordered by creation date (newest first).

    Args:
        limit: Optional limit on number of jobs to return. If None, returns all jobs.

    Returns:
        Formatted string with all INSPECTION job records or error message
    """
    try:
        query = f"""
            SELECT
        ...
        if not rows:
            return "No INSPECTION jobs found in the database."

        # Format results
        result = f"Total INSPECTION jobs found: {len(rows)}\n\n"
        result += "=" * 100 + "\n"
        ...
        error_msg = f"Error retrieving INSPECTION jobs: {str(e)}"
        print(f"[ERROR][get_all_inspection_jobs_tool] {error_msg}")
        return error_msg
