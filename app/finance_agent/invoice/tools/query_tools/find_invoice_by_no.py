"""
Tool for finding invoice by invoice number.

This module provides functionality to query invoices by their invoice number.
"""
...
def find_invoice_by_no(invoice_no: str) -> Dict[str, Any]:
    """
    Find invoice by invoice number.

    This tool queries the database for all invoice items with the given
    invoice number and returns them with client information.

    Args:
        invoice_no: Invoice number to search for (e.g., "INV-JCP-25-01-1")
    ...
    Examples:
        >>> find_invoice_by_no("INV-JCP-25-01-1")
        {
            "success": True,
            "invoice_no": "INV-JCP-25-01-1",
            "items_found": 2,
        ...
        >>> find_invoice_by_no("INV-NOTFOUND")
        {"error": "No invoice found with inv_no='INV-NOTFOUND'"}
    """
    try:
        if not invoice_no:
        ...
        rows = execute_query(
            ...
            WHERE i.{InvoiceFields.INV_NO} = %s
        ...
        print(f"[ERROR][find_invoice_by_no] {error_msg}")
