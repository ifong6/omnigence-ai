...
def find_design_jobs_by_client_tool(client_name: str):
    """
    Find DESIGN jobs by client name. Jobs are searched from Finance.design_job table only.

    args:
        client_name: str - The client/company name to search for

    returns:
        list[dict]: The DESIGN jobs found for this client
    """
    query = f"""
        WITH companies AS (
        ...
        SELECT j.id, j.job_no, j.title, j.status, 'DESIGN' AS type, j.date_created, c.name AS company_name
